...
boot_straps = data_frame(
 strap_number = 1:1000,
 strap_sample = purrr::rerun(1000, boot_sample(final_dataset))
)
boot_straps
boot_straps %>%
...
bootstrap_results = 
  boot_straps %>%
  mutate(models = purrr::map(strap_sample, ~lm(log_day ~ ageyear + evisit_catg + bpsystolic + heartrate + cindex_catg + is30dayreadmit + insurance_catg + respirationrate + temperature + bpdiastolic + gender + marital_catg + race_catg + facilityname, data = data)
  ),
