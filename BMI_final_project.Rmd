...
```{r}
###the function to replace the missing data with median
...
#for numeric vectors, fill in missing values with the mean or median of non-missing values
  vec[is.na(vec)] = round(median(vec,na.rm = TRUE),2)
...
data = read_excel("GHProject_Dataset.xlsx") %>%
  ...
  mutate(bmi = replace_missing_value(bmi)) %>% 
  #filter out extreme temperatures
  filter(temperature < 48, temperature > 29) %>% 
  #filter out concerning bmi values
  filter(!(bmi > 65), !(bmi < 12)) %>% 
...
knitr::kable(dplyr::bind_rows(
  ...
  c(variable = "bmi",sum3),#688 missing values and long tail
...
# na.omit in order to have the correlation coefficients to be calculated
data_no_na = data %>% na.omit()

#correlation matrix
pairs(~log(losdays2) + ageyear + bmi + bpsystolic + o2sat+ temperature +log(heartrate) +log(respirationrate)+ bpdiastolic, data=data_no_na, lower.panel = panel.smooth, upper.panel = panel.cor,main = "Correlation matrix for continuous data" )
...
###try to separate the data into "alone" and "not alone"
data_alone = data[data$marital_catg == "Alone",]
data_notalone = data[data$marital_catg == "Not alone",]

#fit bmi again with nas omitted
data_bmiomit = data %>%
  na.omit(bmi)
summary(lm(log(losdays2) ~ bmi_catg, data = data_bmiomit))
...
summary(lm(log(losdays2) ~ ageyear + mews_catg + as.factor(cindex) + as.factor(evisit) + 
    gender + insurance_catg + marital_catg + bmi_catg + facilityname, data = data))
...
```{r}
# model based on variables cody suggested
subset_data = data%>%
  select(patientid, losdays2, date, mews_catg, cindex_catg, evisit_catg, race_catg, religion, marital_catg, insurance_catg)
  

smaller_model = lm(log(losdays2) ~ ageyear + mews_catg + 
                 as.factor(cindex) + as.factor(evisit) + as.factor(icu_flag) + 
                 gender + race_catg + insurance_catg + marital_catg,
                 data = subset_data)
HH::vif(smaller_model) #all vifs are well under 5, no concerns of multicollinearity


#not sure how to interpret these
ols_stepaic_both(smaller_model)

step(smaller_model, direction='both')
```
