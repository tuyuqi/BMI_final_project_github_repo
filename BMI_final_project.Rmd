...
data$mews_catg<-ifelse(data$mews==0|data$mews==1,1,
                       ifelse(data$mews==2|data$mews==3, 2,3))
...
data$cindex_catg<-ifelse(data$cindex==0,"normal",
                         ifelse(data$cindex==1|data$cindex==2,"mild","moderate to severe"))
table(data$cindex_catg)

#evisit
table(data$evisit)
data$evisit_catg<-ifelse(data$evisit==0|data$evisit==1,1,2)
...
data$race_catg<-ifelse(data$race=="White","White",
                ifelse(data$race=="African Amer/Black","African Amer/Black",
                "Other race"))
...
data$marital_catg <- ifelse(data$maritalstatus=="Civial            Union"|data$maritalstatus=="Married"|data$maritalstatus=="Civial Union","Not alone","Alone")
table(data$marital_catg)
#InsuranceType: patient???s insurance
table(data$insurancetype)
data$insurance_catg <- ifelse(data$insurancetype=="Private","Private","Public")
...
hist(log(data$heartrate)) #see some improvement
hist(log(data$respirationrate)) #see some imporvement
```
...
whole_model = lm(log(losdays2)~ageyear+bmi+bpsystolic+o2sat+temperature+
                 log(heartrate)+log(respirationrate)+bpdiastolic+
                 religion+as.factor(mews_catg)+
                 as.factor(cindex_catg)+as.factor(evisit_catg)+as.factor(icu_flag)+
                 as.factor(gender)+as.factor(race_catg)+insurance_catg+marital_catg,
                 data = data)


#try to separate the dataset with icu_flag==0 and icu_flag==1
model_icu0 = lm(log(losdays2)~ageyear+bmi+bpsystolic+o2sat+temperature+
                 log(heartrate)+log(respirationrate)+bpdiastolic+
                 religion+as.factor(mews_catg)+
                 as.factor(cindex_catg)+as.factor(evisit_catg)+
                 as.factor(gender)+as.factor(race_catg)+insurance_catg+marital_catg,
                 data = data_icu0)
model_icu1 = lm(log(losdays2)~ageyear+bmi+bpsystolic+o2sat+temperature+
                 log(heartrate)+log(respirationrate)+bpdiastolic+
                 religion+as.factor(mews_catg)+
                 as.factor(cindex_catg)+as.factor(evisit_catg)+
                 as.factor(gender)+as.factor(race_catg)+insurance_catg+marital_catg,
...
#try to eliminate all the missing values and fit the model
data_omit_miss = na.omit(data)
model_omit_miss = lm(log(losdays2)~ageyear+bmi+bpsystolic+o2sat+temperature+
                 log(heartrate)+log(respirationrate)+bpdiastolic+
                 religion+as.factor(mews_catg)+
                 as.factor(cindex_catg)+as.factor(evisit_catg)+as.factor(icu_flag)+
                 as.factor(gender)+as.factor(race_catg)+insurance_catg+marital_catg,
...
#back<-ols_step_backward(whole_model, prem = 0.2)
forward<-ols_step_forward(whole_model, prem = 0.2)
#step<-ols_stepwise(whole_model, prem = 0.2)
